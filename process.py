...
class MyClassifier:
    ...
    def __init__(self) -> None:
        ...
        models_dir = os.path.join(curr_dir, "dobble_master_race/models_save")
        ...
        hist_args = {"saved_kmeans": kmeans_path}
        transformer = HuMomentsColorsHistogram(6, 10, hist_args, {})
        with open(scaler_path, "rb") as scaler_file:
            scaler = pickle.load(scaler_file)
        with open(classifier_path, "rb") as classifier_file:
            classifier = pickle.load(classifier_file)

        self.pipeline = make_pipeline(transformer, scaler, classifier)
    ...
    def classify_image(self, image_path: str) -> int:
        ...
        cls_id = self.pipeline.predict(np.array([img]))[0]
        return cls_id
