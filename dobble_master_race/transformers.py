...
class ColorsHistogram(BaseEstimator, TransformerMixin):
    def __init__(self, nb_colors: int, white_threshold: int = 240,
                 shape_inners: bool = True, max_samples: int=50000,
                 median_blur: bool = True) -> None:
        super().__init__()
    ...
    def fit(self, X, y=None):
        ...
        if self.shape_inners:
            masks = np.array([
                mask_with_inner(img, self.white_threshold)
                for img in X
            ], dtype=object)
        else:
            masks = np.array([
                img.sum(axis=-1) < (self.white_threshold * 3)
                for img in X
            ], dtype=object)
    ...
    def transform(self, X, **kwargs):
        ...
        color_maps = np.array([
            self.clustering.predict(img[mask])
            for img, mask in zip(X, masks)
        ], dtype=object)

        return np.array([get_hist(color_map, self.nb_colors) for color_map in color_maps], dtype=object)
...
class HuMoments(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, **kwargs):
        masks = np.array([
            np.array(img.sum(axis=-1) < (self.white_threshold * 3),
                     dtype=np.uint8)
            for img in X
        ], dtype=object)
        ...
        return np.array([cv.HuMoments(mask_moments)[:self.nb_moment] for mask_moments in moments]).reshape(X.shape[0], -1)
...
class AspectRatio(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, **kwargs):
        ...
        bounding_rects = np.array([cv.minAreaRect(contour[0]) for contour in contours], dtype=object)
...
class HuMomentsColorsHistogram(BaseEstimator, TransformerMixin):
    def __init__(self, nb_moment: int, nb_colors: int, hist_args: dict[str, Any], hu_args: dict[str, Any]):
        super().__init__()
